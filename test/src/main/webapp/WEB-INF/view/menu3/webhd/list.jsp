...
<div class="bodyFrame2">
    <div class="body-title">
          <h3><span class="glyphicon glyphicon-book"></span> 회사 웹하드 입니다. </h3>
    </div>
    
    <div class="alert alert-info">
        <i class="glyphicon glyphicon-info-sign"></i> 중요한 자료 및 문서 등을 올리세요.
    </div>
...
<%-- 
    ...
    <c:forEach var="dto" items="${list}">
                    <tr>
                        ...
                        <td class="text-center">${dto.s_num}</td>
                        <td class="text-center">${dto.created}</td>
                        <td class="text-center">${dto.hitCount}</td>
                    </tr>
     </c:forEach>                    
        ...
        <div style="clear: both;">
        		<div style="float: left; width: 20%; min-width: 85px;">
        		    <button type="button" class="btn btn-default btn-sm wbtn" onclick="javascript:location.href='<%=cp%>/webhd/list';">새로고침</button>
        		</div>
...
<%-- <c:if test="${sessionScope.staff.s_num=='admin'}"> --%>
        		    <button type="button" class="btn btn-primary btn-sm bbtn" onclick="javascript:location.href='<%=cp%>/webhd/created';"><span class="glyphicon glyphicon glyphicon-pencil"></span> 글쓰기</button>
<%-- </c:if> --%>        		    
        		</div>
        </div>
