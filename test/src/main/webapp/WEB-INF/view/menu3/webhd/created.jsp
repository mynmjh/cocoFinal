...
<script type="text/javascript">
  function check() {
        ...
        var mode="${mode}";
    	if(mode=="created")
    		f.action="<%=cp%>/webhd/created";
    	else if(mode=="update")
    		f.action="<%=cp%>/webhd/update";
					...
					<tbody id="tb">
						<tr>
							...
							<td class="td2 col-md-5 col-sm-5">
								<p class="form-control-static">${sessionScope.staff.s_num}</p>
							</td>
							<%-- 
						...
						<tr>
							<td colspan="4" class="td4"><textarea name="content"
									class="form-control" rows="15" required="required">${dto.content}</textarea>
							</td>
						</tr>
						...
						<tr>
							...
							<td colspan="3" class="td3"><input type="file" name="upload"
								class="form-control input-sm" style="height: 35px;"></td>
						</tr>
					</tbody>
					...
					<c:if test="${mode=='update'}">
						<c:forEach var="vo" items="${listFile}">
							<tr id="f${vo.fileNum}">
								...
								<td colspan="3" class="td3">${vo.originalFilename} | <a
						...
						<tr>
							<td colspan="4" style="text-align: center; padding-top: 15px;">
								...
								<button type="button" class="btn btn-danger"
									onclick="javascript:location.href='<%=cp%>/webhd/list';">
