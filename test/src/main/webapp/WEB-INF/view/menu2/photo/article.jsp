...
//댓글 추가
function sendReply() {
	var uid="${sessionScope.staff.s_num}";
	if(! uid) {
		login();
		return false;
	}
...
<c:if test="${sessionScope.staff.s_num==dto.s_num}">
  ...
  var url = "<%=cp%>/photo/delete?" + params;
...
<c:if test="${sessionScope.staff.s_num==dto.s_num}">
  ...
  var page = "${page}";
  var params = "num="+num+"&page="+page;
  var url = "<%=cp%>/photo/update?" + params;
                     ...
                     <tr>
                         <td colspan="2">
                              ...
                              <c:if test="${not empty preReadDto }">
                                  <a href="<%=cp%>/photo/article?${params}&num=${preReadDto.num}">${preReadDto.subject}</a>
                              </c:if>					
                         </td>
                     </tr>
                     <tr>
                         <td colspan="2" style="border-bottom: #d5d5d5 solid 1px;">
                              ...
                              <c:if test="${not empty nextReadDto }">
                                  <a href="<%=cp%>/photo/article?${params}&num=${nextReadDto.num}">${nextReadDto.subject}</a>
                              </c:if>
                         </td>
                     </tr>                                          
                </tbody>
...
<c:if test="${sessionScope.staff.s_num==dto.s_num}">	                		    
                		...
                		<td align="right">
                		    <button type="button" class="btn btn-default btn-sm wbtn" onclick="javascript:location.href='<%=cp%>/photo/list?${params}';"> 목록으로 </button>
                		</td>
                	</tr>
                </tfoot>
            </table>
